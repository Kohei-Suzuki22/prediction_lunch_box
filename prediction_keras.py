...
kcal = make_input_data(input_kcal,affect_length)
d = make_input_data(input_days,affect_length)
month = make_input_data(input_month,affect_length)
...
kcal = reshape_input(kcal)
d = reshape_input(d)
month = reshape_input(month)
...
# for i in range(len(factors_y)):
#   factors = np.append(factors,[factors_y[i],d[i],kcal[i],curry[i],amuse[i],zeroRain[i],temperature[i],weather_fine[i],weather_sun[i],weather_cloud[i],weather_sin_cloud[i],weather_rain[i],weather_snow[i],weather_thunder[i]])

# for i in range(len(factors_y)):
#   factors = np.append(factors,[factors_y[i],d[i],kcal[i],temperature[i]])
for i in range(len(factors_y)):
  factors = np.append(factors,[factors_y[i],d[i],kcal[i],temperature[i],month[i],amuse[i]])

# for i in range(len(factors_y)):
#   factors = np.append(factors,[factors_y[i],d[i],temperature[i]])

# for i in range(len(factors_y)):
#   factors = np.append(factors,[factors_y[i],kcal[i],curry[i],amuse[i],zeroRain[i],temperature[i],weather_fine[i],weather_sun[i],weather_cloud[i],weather_sin_cloud[i],weather_rain[i],weather_snow[i],weather_thunder[i]])
...
def make_model(factors,answers,input_dim,non_linear_factors):
  model = Sequential()
  ...
  model.fit(factors,answers,batch_size=1,epochs=6000,validation_split=0.2, callbacks= [early_stopping])
  ...
  non_linear_y = answers - pred


  model2 = Sequential()
  model2.add(Dense(20,kernel_initializer='he_normal'))
  # model2.add(BatchNormalization())
  model2.add(Activation('tanh'))
  model2.add(Dense(output_dim=1,input_dim=20))
  ...
  non_linear_factors = non_linear_factors.reshape(-1,1)
  # ipdb.set_trace()

  model2.fit(non_linear_factors[affect_length:][150:],non_linear_y[150:],batch_size=1,epochs=6000,validation_split=0.2, callbacks= [early_stopping])
  # model2.fit(non_linear_factors[affect_length:][150:],non_linear_y[150:],batch_size=1,epochs=6000,validation_split=0.2, callbacks= [])

  pred2 = model2.predict(non_linear_factors[affect_length:])
...
pred = make_model(factors,answers,n_in,input_curry)
# pred = model.predict(factors)
...
def show_graph(x,pred,expect,affect_length):
  ...
  # non_linear_y = expect[affect_length:] - pred
...
detrend_expect = signal.detrend(input_y)
detrend_expect = detrend_expect.reshape(-1,1)

# show_graph(days,pred,detrend_expect,affect_length)



'''トレンド除去後、学習データと予測データの差分グラフ'''
non_linear_y = detrend_expect[affect_length:] - pred
